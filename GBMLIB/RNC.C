/*RNC decompression*/
/*Code by Robert Northen*/
/*De-compiled by Lab 313*/
#include <errno.h>
#include <stdio.h>
#include <stdlib.h>
#include <string.h>
...
int find_matches(vars_t* v)
{
    ...
    first_word = peek_word_be(v->pack_block_start, 0);
    offset = v->mem2[first_word & 0x7FFF];
    ...
    while (1)
    {
        ...
        restore = v->mem4[offset & 0x7FFF];
        min_offset = v->last_min_offset;
        ...
        {
            max_count = v->mem5[offset & 0x7FFF];
            ...
            if (max_count <= min_offset)
            {
                ...
                max_size = v->pack_block_end - v->pack_block_start;
                if (max_count == match_offset)
                {
...
void encode_matches(vars_t* v, uint16 w)
{
    ...
    while (1)
    {
        ...
        buffer_word = peek_word_be(v->pack_block_start, 0);
...
void proc_6(vars_t* v)
{
    ...
    {
        size_to_read = 0xFFFF - v->dict_size - v->pack_block_pos;
...
int do_search(vars_t* v, size_t input_size, int save)
{
    ...
    for (i = 0; i < input_size - RNC_HEADER_SIZE; )
    {
...
void procRNC(unsigned char* source, unsigned char* dest, int size)
{
    vars_t* v = init_vars();
    v->file_size = size;
    v->input = source;
